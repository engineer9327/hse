...
async function fetchAwsStationCoords() {
  ...
  // AWS 응답에는 주소가 없으므로 KMA_STATIONS(지점번호 동일 체계)와 매칭해 보완
  const addrById = {};
  (typeof KMA_STATIONS !== 'undefined' ? KMA_STATIONS : []).forEach(s => { addrById[String(s.id)] = s.addr; });
  Object.values(coords).forEach(c => {
    if (!c.addr && addrById[String(c.id)]) c.addr = addrById[String(c.id)];
  });
